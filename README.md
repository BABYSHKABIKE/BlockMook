...
2. Распакуй **весь архив**. Открой **Install.cmd** для установки и ярлыка либо **Start.cmd** для переносимого запуска.
...
По умолчанию крестик скрывает окно в трей. **Отключить** останавливает подключение; **Выйти** в меню трея завершает приложение. Для работы нужны Windows 10/11 **x64** и .NET Framework 4.8. Доступ зависит от провайдера: статус частичного доступа не выдаётся за успешное подключение.
...
| **Настройки** | Профиль, трей, восстановление, запуск с Windows, ярлык и интерфейс. |
| **Диагностика** | Проверить соединения, получить рекомендации, сохранить отчёт или проверить профили. |
| **О приложении** | Исходные проекты, лицензии, ограничения и журнал. |

Автоматический режим пробует до трёх профилей. Выбранный профиль сохраняется для соответствующего набора сервисов после **двух успешных проверок**. Быстрая диагностика сохраняет текущее подключение. Расширенная проверка профилей перебирает способы и затем отключает подключение.
...
Подробнее: [трей, восстановление, диагностика и установка](docs/desktop.md).
...
В версии 1.2 обновление скачивается как ZIP: выйди из старой версии через трей, распакуй архив и открой `Install.cmd` либо `Start.cmd`. Автоматической замены запущенной программы пока нет. Настройки сохранятся. Проверка SHA-256 подтверждает целостность загрузки; сборка пока не имеет цифровой подписи издателя.
...
Проверки доступа обращаются к выбранным сервисам и Microsoft. Для Google Meet также выполняется UDP/STUN-запрос к Google. BlockMook **не является VPN** и не скрывает IP-адрес. Успешная проверка не гарантирует вход в аккаунт, видео или звонок. DNS и прокси приложение не меняет. Автозапуск включается отдельно в настройках. При включённом автовосстановлении проверки выполняются в фоне. Локальные отчёты могут содержать пути и диагностические сообщения — просмотри их перед публикацией.
