...
**BlockMook** — открытое Windows-приложение для управления zapret: YouTube и Discord, автоматический подбор подключения и понятная диагностика. Без аккаунта и установки отдельного zapret.
...
| --- | --- |
| **Подключение** | Выбрать YouTube и Discord, подключиться, проверить доступность. |
...
В версии 1.0 обновление скачивается как ZIP: после загрузки распакуй его в новую папку и открой `Start.cmd`. Автоматической замены запущенной программы пока нет. Настройки сохранятся. Проверка SHA-256 подтверждает целостность загрузки; сборка пока не имеет цифровой подписи издателя.
...
Проверки доступа обращаются к YouTube, Discord и Microsoft. BlockMook **не является VPN** и не скрывает IP-адрес. Успешная проверка не гарантирует вход в аккаунт, видео или звонок. DNS, прокси и автозапуск Windows приложение не меняет. Локальные отчёты могут содержать пути и диагностические сообщения — просмотри их перед публикацией.
