...
internal sealed class ServiceDefinition {
    ...
    internal string Name,Category,Support,Description,SearchAliases;
...
internal static class Services {
    ...
    internal static readonly ServiceDefinition[] Items={
        new ServiceDefinition {Bit=1,Index=0,Name="YouTube",Category="Видео",Support="Базовый профиль",Description="Сайт и видео. Проверка подтверждает ответ сервиса; воспроизведение проверьте в браузере.",Domains=new[]{"youtube.com","youtu.be","googlevideo.com","ytimg.com","youtubei.googleapis.com","youtube.googleapis.com","youtube-nocookie.com","yt3.ggpht.com","yt4.ggpht.com","yt3.googleusercontent.com","youtubeembeddedplayer.googleapis.com"}},
        new ServiceDefinition {Bit=2,Index=1,Name="Discord",Category="Общение",Support="Базовый профиль",Description="Приложение и голос. Проверка API, ресурсов клиента и WebSocket. Голосовой звонок — вручную.",Domains=new[]{"discord.com","discord.gg","discordapp.com","discordapp.net","discord.media","discordstatus.com"}},
        new ServiceDefinition {Bit=16,Index=5,Name="Google Meet",Category="Видеовстречи",SearchAliases="мит",Support="Экспериментально",Description="Сайт, вход и медиасерверы Google. Проверка TLS и UDP/STUN. Видео, звук и вход в аккаунт проверьте во встрече.",Domains=new[]{"meet.google.com","accounts.google.com","apis.google.com","meetings.googleapis.com","hangouts.googleapis.com","apps.google.com","docs.google.com","clients2.google.com","clients4.google.com","clients6.google.com","www.gstatic.com","fonts.gstatic.com","lh3.googleusercontent.com","workspace.turns.goog","meet.turns.goog"},TlsHosts=new[]{"meet.google.com","accounts.google.com","meet.turns.goog"}},
        new ServiceDefinition {Bit=4,Index=3,Name="Signal",Category="Мессенджеры",Support="Экспериментально",Description="Домены Signal и ссылки. Проверка TLS сервера сообщений и хранилища. Доставку сообщений и звонки — в приложении; отдельной UDP-стратегии нет.",Domains=new[]{"signal.org","signal.art","signal.group","signal.link","signal.me","signal.tube","signalcaptchas.org"},TlsHosts=new[]{"chat.signal.org","storage.signal.org"}},
    ...
    internal static ServiceDefinition[] Selected(int mask){if(!ValidMask(mask))throw new ArgumentException("Выберите хотя бы один сервис");return Items.Where(s=>(s.Bit&mask)!=0).ToArray();}
    ...
    internal static ServiceDefinition[] Search(string query){string term=(query??"").Trim();return Items.Where(s=>(s.Name+" "+s.Category+" "+s.SearchAliases).IndexOf(term,StringComparison.OrdinalIgnoreCase)>=0).ToArray();}
}
}
